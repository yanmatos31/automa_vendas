...
if response.status_code == 200:
    ...
    offer_elements = soup.find_all('div', class_='flex flex-col rounded-2 bg-neutral-high-100 dark:bg-neutral-low-300 md:justify-between lg:hover:shadow-md lg:hover:transition-all lg:hover:ease-in h-full')

    # Pegando apenas os 10 primeiros elementos
    offers = offer_elements[:10]
    ...
    for offer in offers:
        link_element = offer.find('a', class_='no-underline [&>*]:text-primary-400 [&>*]:hover:text-primary-300 [&>*]:active:text-primary-200 [&>*]:dark:text-primary-200 [&>*]:hover:dark:text-primary-100 [&>*]:active:dark:text-primary-100 flex h-full flex-col justify-between rounded-2 border border-neutral-high-300 dark:border-neutral-low-500 lg:border-none')
        ...
        if offer_response.status_code == 200:
            ...
            title_element = offer_soup.find('h1', class_='text-neutral-low-300 dark:text-neutral-high-100 text-lg font-bold lg:text-2xl')
            title = title_element.text.strip() if title_element else ''

            price_element = offer_soup.find('span', class_='text-2xl font-bold tracking-normal lg:text-4xl whitespace-nowrap text-primary-500 dark:text-primary-100')
